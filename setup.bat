...
:configured
"%FOOTBOY_PYTHON%" "%~dp0scripts\bootstrap.py" setup %*
exit /b %errorlevel%

:virtualenv
"%~dp0.venv\Scripts\python.exe" "%~dp0scripts\bootstrap.py" setup %*
exit /b %errorlevel%

:launcher
py -3 "%~dp0scripts\bootstrap.py" setup %*
exit /b %errorlevel%

:python
python "%~dp0scripts\bootstrap.py" setup %*
exit /b %errorlevel%
