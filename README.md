...
当前 `0.1.3` 补齐真实直播小计时框的 OCR 定位和细长数字识别，保留比赛源直连、FFmpeg 崩溃处理、Chromium HLS 播放，以及 P3 的周期 OCR 复核、连续两次漂移确认、B 站重解析及第三方错误重嗅探。音频 GCC-PHAT 模块尚未接入控制流程，时钟跳变处逐帧精细化留待后续阶段。
...
两个 B 站房间的真实取流、视频与音频复制、手动偏移、恢复及停止重连结果见 [0.1.2 验证记录](docs/validation-0.1.2.md)；该轮约 3 分钟的功能检查关闭 OCR。视频样本 A非足球赛事赛事重播的真实 OCR、同一路媒体独立双连接与手动优先结果见 [0.1.3 验证记录](docs/validation-0.1.3.md)。识别仍可能漏读或误读，必须通过至少 3 帧连续走表校验；赛间没有计时时显示“未对齐”。不同来源的同场足球同步精度、连续 2 小时运行、实际长延迟双路缓冲及手机 Safari 连播 30 分钟仍待验收。
